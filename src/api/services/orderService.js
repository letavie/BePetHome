...
const createOrder = async (data) => {
  try {
    ...
    const totalPrice =
      orderDetails.reduce((acc, orderDetail) => acc + orderDetail.price, 0) +
      12000;
    let order = {};
    if (paymentMethod == "OP") {
    ...
    if (order._id) {
      const userUpdate = await _User.findByIdAndUpdate(userId, {
...
const sendOrderEmail = async (order, user) => {
  ...
  const listProduct = order.orderDetails
    .map(
      (
        o
      ) => `<table style="width: 100%; margin-top: 15px; border-bottom: 1px solid #e5e5e5;">
        ...
        <tr>
            <td id="main" style="padding: 60px; border-bottom: 5px solid #e5e5e5;">
                ...
                <div style="text-align: right; font-size: 16px; font-weight: 600; line-height: 1.4; color: #555;">
                    <div>Giá: <span style="margin-left: 15px;">${
                      order.totalPrice - 12000
                    } ₫</span></div>
                    <div>Tiền ship: <span style="margin-left: 15px;">12000 ₫</span></div>
                    <div>Tổng: <span style="margin-left: 15px;">${
                      order.totalPrice
                    } ₫</span></div>
                </div>
            </td>
                    ...
                    <tr>
                        <td style="vertical-align: top; padding-right: 20px;">
                            ...
                            <div style="font-size: 16px; font-weight: 600; line-height: 1.4; color: #555;width:300px">
                                ${order.shipping.addressShipping.street}, ${
      order.shipping.addressShipping.district
